	...
	<div class="row">
		<div class="col-lg-12 col-sm-12">
			<div class="pull-right">
				<a href="${contextPath}/admin/member/form_1/create"
					class="btn btn-success">新增</a> <a href="${contextPath}/admin/excel"
					class="btn btn-success">导出Excel</a> <a href="form/import_excel"
					class="btn btn-success">导入Excel</a>
			</div>
			<form action="${contextPath}/admin/member/list" class="form-inline"
				id="filterForm" name="QueryForm">
				...
				<div class="form-group">
					<input type="text" class="form-control" placeholder="职工姓名"
						name="chName1" value="${paginated.chName1}" />
				</div>
				<div class="form-group">
					<input type="text" class="form-control" placeholder="公司名称"
						name="companyName1" value="${paginated.companyName1}" />
				</div>
				<div class="form-group">
					<input type="text" class="form-control" placeholder="手机号"
						name="mobile1" value="${paginated.mobile1}" />
				</div>
				<div class="form-group">
					<input type="text" class="form-control" placeholder="邮箱"
						name="email1" value="${paginated.email1}" />
				</div>
		...
		<table class="table table-striped">
			<thead>
				<tr>
					...
					<th id="companyName" name="companyName">公司名称&nbsp;<i
						class="glyphicon glyphicon-chevron-up"></i>
					</th>
			...
			<tbody>
				...
				<c:forEach items="${paginated.list}" var="m">
					<tr>
						...
						<td><c:if
								test="${SPRING_SECURITY_CONTEXT.authentication.principal.username == m.companyUsername}">
								<a href="${contextPath }/admin/member/form_1/${m.uuid }">编辑</a>
							</c:if> <a href="${contextPath }/admin/member/detail/${m.uuid }">查看</a>
							<a href="${contextPath }/admin/member/log/${m.uuid }">日志</a> <c:if
								test="${SPRING_SECURITY_CONTEXT.authentication.principal.username == m.companyUsername}">
								<a onclick="return confirm('你确定删除职工[${m.chName}]')"
									href="${contextPath }/admin/member/delete/${m.uuid }">删除</a>
							</c:if>
						</td>
					</tr>
				</c:forEach>
