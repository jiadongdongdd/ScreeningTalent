	...
	<div id="ban" class="container">
		<div class="jumbotron">
			<div class="lead">
				...
				<c:if test="${result eq 'activeSuccess' }">
					<p>激活成功,欢迎您来到筛子网！</p>
				</c:if>
				<c:if test="${result eq 'fail' }">
					<p>激活链接不存在！</p>
				</c:if>
				<c:if test="${result eq 'exist' }">
					<p>你已激活账户,无需重复激活！</p>
				</c:if>
				<c:if test="${result eq 'companySuccess' }">
					<p>注册成功,欢迎您来到筛子网！</p>
					<p>企业正在审核,审核通过后会邮件发给您!</p>
				</c:if>
				<c:if test="${result eq 'ReCreateComfail' }">
					<p>重新注册企业链接不存在！</p>
				</c:if>
				<c:if test="${result eq 'reCreateComPass' }">
					<p>该企业已经审核成功,无需重新注册！</p>
				</c:if>
				<c:if test="${result eq 'reCreateComSuccess' }">
					<p>重新注册成功！</p>
					<p>企业正在审核,审核通过后会邮件发给您!</p>
				</c:if>
			</div>
