var FancyForm = function () {
    return {
        ...
        setup: function () {
            ...
            $('body').on("keyup blur", a.inputs, function () {
                var c = $(this);
                a.checkVal(c);
            });
...
var searchAjax = function (name, id, isAdd) {
    setSelectTips();
    amountAjax(name, id, isAdd, true);
};

function amountAjax(name, id, isAdd, isBasic) {
    if (isBasic) {
        var flag = "tag_" + id;
        var basicFlag = "basic_tag_" + id;
        if (isAdd) {
            $('form#tagForm').append('<input id="' + flag + '" name="tags" value="' + id + '"/>');
            $('div#basicTagsDiv').append('<input id="' + basicFlag + '" class="hidden" name="selectedBasicTags" value="' + id + '"/>');
        } else {
            $('#' + flag).remove();
            $('#' + basicFlag).remove();
        }
    ...
    $('#tagForm').ajaxSubmit({
        url: "../../query_question_by_tags",
        dataType: "json",
        type: "post",
        success: function (data) {
            /* $('#totalQuestion').html('');
             $('#totalQuestion').html('当前试题总数：' + data.amount);*/
...
(function () {
    ...
    $('body').on('click', '.default-tag a', function () {
        var $this = $(this),
